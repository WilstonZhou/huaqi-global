...
countries.forEach(function (c) {
  ...
  write('country/' + c.slug + '.html', lib.buildPage({
    ...
    schemas: schemas.concat([lib.faqSchema(c.faqs), lib.howToSchema(c.name + '公司注册流程', c.flow)]),
    body: body
  }));
});
...
(function () {
  ...
  var body = lib.hero({
    ...
    title: '热门国家公司注册',
    subtitle: '覆盖全球 126 个国家及地区,核心 60+ 国家/地区可直办。下方为 ' + countries.length + ' 大热门市场独立指南,包含监管机构、公司类型、注册条件、材料、流程与常见问题。'
  });
  ...
  write('country/index.html', lib.buildPage({
    ...
    desc: '华企环球热门国家公司注册指南:新加坡、美国、香港、马来西亚、泰国、英国、日本、韩国、迪拜、越南、印尼等热门市场,监管机构、注册条件、流程与费用参考一页直达,全程代办。',
